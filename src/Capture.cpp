...
PcapCapture::PcapCapture(const std::string &interface,
  ...
  if (pcap_compile(pcap_handle_, &fp, filter.c_str(), 0,
    ...
    throw std::runtime_error("BPF compile error: " +
  ...
  if (pcap_setfilter(pcap_handle_, &fp) == -1) {
    pcap_close(pcap_handle_);
    pcap_freecode(&fp);
    throw std::runtime_error("BPF setfilter error: " +
                             std::string(pcap_geterr(pcap_handle_)));
  }
  pcap_freecode(&fp);
}
...
void PcapCapture::start_capture(CallbackType callback) {
  ...
  processing_thread_ = std::thread([this]() { process_packets(); });
}
...
void PcapCapture::packet_handler(u_char *user, const struct pcap_pkthdr *h,
                                 const u_char *bytes) {
...
void PcapCapture::process_packets() {
  while (is_capturing_ || !packet_queue_.empty()) {
    ...
    queue_cv_.wait(
        lock, [this]() { return !packet_queue_.empty() || !is_capturing_; });
